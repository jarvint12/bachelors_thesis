...
def count_p_values(values, directory, treshold, column, headers, larger_better):
    if values.skip!=None:
...
def main():
    ...
    if not values.skip_muffinn:
        for treshold_m in values.treshold_m:
            muffinn_p1, muffinn_p2=count_p_values(values, values.muffinn, treshold_m, 9,1, True)
    ...
    if not values.skip_dendrix:
        for treshold_d in values.treshold_d:
            dendrix_p1, dendrix_p2=count_p_values(values, values.dendrix, treshold_d, 0, 0, True)
    ...
    if not values.skip_onco:
        for treshold_o in values.treshold_o:
            oncodrive_p1, oncodrive_p2=count_p_values(values, values.oncodrive, treshold_o, 1, 1, False)
